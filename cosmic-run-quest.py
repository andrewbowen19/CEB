...
print('Starting GC for loop')
for index, row in GCs.iterrows():
	sigma_v = row['sigma_v']#pulling velocity dispersion values from dataframe
	Z = row['[Fe/H]_x']#Metallicity for GCs
	Nbin = row['Nbin']#Nbin -- using 40k per cluster, but will likely 
	...
	if np.isfinite(row['sigma_v']) and np.isfinite(gc_age):
		...
		print('running with feed value (2.28 km/s):', row['ID_x'])
		GC_run = cosmic_sampler(13000, int(Nbin), Z, np.random.uniform(1,3), np.random.randint(1, 100))
	print(GC_run)
...
for index, row in OCs.iterrows():
	...
	if np.isfinite(row['Age']):
		...
		if row['Fe/H'] != -99.99:
			OC_run = cosmic_sampler(oc_age, int(row['Nbin']), row['Fe/H'], row['sigma_v'], np.random.randint(1, 100))#using random # for seed to give cosmic when generating a pop of binaries
			print('Webda:', index)
			print(OC_run)
		else:
	...
	# Goes through Solaris age-metallicity combos to perform cosmic_sampler on each Solaris cluster
		...
		print('Solaris:', index)
		OC_run = cosmic_sampler(row['t'], int(row['Nbin']), row['[FeH]'], row['sigma_v'], np.random.randint(1, 100))
	...
	# Goes through Piskunov age-metallicity combos ... "          "
		...
		OC_run = cosmic_sampler(row['log(t[yr])K'], int(row['Nbin']), row['[Fe/H]K'], row['sigma_v'], np.random.randint(1, 100))
